...
class SppagebuilderAddonArticles2 extends SppagebuilderAddons
{
    ...
    public function render()
    {
        ...
        $show_last_modified_date = (isset($settings->show_last_modified_date)) ? $settings->show_last_modified_date : 1;
        ...
        if (count((array) $items)) {
            $output .= '<div class="sppb-addon sppb-addon-articles article-list ' . $class . '">';
            ...
            foreach ($items as $key => $item) {
                $output .= '<div class="col-xl-' . round(12 / $columns) . '">';
                $output .= '<div class="sppb-addon-article article">';


        $output .= '<div class="article-intro-image">';
                if (!$hide_thumbnail) {
                    $image = '';
                    if ($resource == 'k2') {
      ...
      $output .= '</div>';
                ...
                $output .= '<div class="sppb-article-info-wrap article-body">';
                $output .= '<div class="article-header"><h2><a href="' . $item->link . '" itemprop="url">' . $item->title . '</a></h2></div>';

                if ($show_author || $show_category || $show_date || $show_last_modified_date) {
                    $output .= '<div class="article-info">';
                    ...
                    if ($show_date) {
                        ...
                        $output .= '<span class="published" itemprop="datePublished">' . $date_text . $date . '</span>';
                    }
                    ...
                    if ($show_last_modified_date) {
                        ...
                        $output .= '<span class="published" itemprop="datePublished">' . $modify_text . $modify_date . '</span>';
                    }
                    ...
                    if ($show_category) {
                        ...
                        $output .= '<span class="category-name"><a href="' . $item->catUrl . '" itemprop="genre">' . $item->category . '</a></span>';
                    }

                    if ($show_author) {
                        $author = ($item->created_by_alias ? $item->created_by_alias : $item->username);
                        $output .= '<span class="createdby" itemprop="name">' . $author . '</span>';
                    }
                ...
                if ($show_intro) {
                    $output .= '<div class="article-introtext"><p>' . mb_substr($item->introtext, 0, $intro_limit, 'UTF-8') . '</p></div>';
                }

                if ($show_readmore) {
                    $output .= '<div class="readmore"><a href="' . $item->link . '" itemprop="url">' . $readmore_text . '</a></div>';
                }
